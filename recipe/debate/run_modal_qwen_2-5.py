...
@app.function(
    ...
    timeout=24 * 60 * MINUTES,#24 * 60 * MINUTES,
    scaledown_window = 2 * MINUTES,
    env={
        "PYTORCH_CUDA_ALLOC_CONF": "expandable_segments:True",
        "HYDRA_FULL_ERROR": "1",
        "SGLANG_LOG_LEVEL": "debug",
        "SGLANG_ROUTER_WORKER_STARTUP_TIMEOUT": "300",
    },
)
def train(*arglist) -> None:
    cmd: list[str] = [
        ...
        "actor_rollout_ref.model.path=Qwen/Qwen2.5-0.5B-Instruct",#Qwen/Qwen3-4B-Instruct-2507",#meta-llama/Llama-3.1-8B-Instruct",#google/gemma-3-12b-it",
        ...
        "trainer.n_gpus_per_node=4", # 4
        "trainer.nnodes=1",
        "trainer.test_freq=10000000",
        f"trainer.default_local_dir={MODELS_PATH / "qwen_2-5"}",
        ...
        "reward_model.n_gpus_per_node=4", # 4
        "reward_model.nnodes=1",
        "reward_model.micro_batch_size=16",
        "reward_model.model.path=Qwen/Qwen2.5-0.5B-Instruct",#Qwen/Qwen3-4B-Instruct-2507",#google/gemma-3-4b-it",
        ...
        # "+reward_model.rollout.engine_kwargs={}",
        "+reward_model.rollout.limit_images=null",
        "+reward_model.rollout.enable_chunked_prefill=false",
        ...
        "+reward_model.rollout.engine_kwargs.sglang.disable_radix_cache=true",
        "+reward_model.rollout.engine_kwargs.sglang.disable_overlap_schedule=True",
        "+reward_model.rollout.engine_kwargs.sglang.disable_custom_all_reduce=True",
    ]
    if arglist:
        cmd.extend(arglist)

    with chdir(VERL_PATH):
